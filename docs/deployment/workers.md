...
6. Create a FlareAuth public OIDC application and a Sink API resource:
   - Redirect URI: `https://your-sink-domain.example/dashboard/callback`
   - Post logout redirect URI: `https://your-sink-domain.example/dashboard/login`
   - API resource audience: `https://your-sink-domain.example/api`
...
7. Configure Worker variables and secrets:
   ...
   - `NUXT_AUTH_AUDIENCE`: Sink API resource audience.
   - `NUXT_PUBLIC_AUTH_ISSUER`: FlareAuth issuer URL for the dashboard.
   - `NUXT_PUBLIC_AUTH_CLIENT_ID`: FlareAuth public OIDC client ID.
   - `NUXT_PUBLIC_AUTH_REDIRECT_URI`: `https://your-sink-domain.example/dashboard/callback`.
   - `NUXT_PUBLIC_AUTH_POST_LOGOUT_REDIRECT_URI`: `https://your-sink-domain.example/dashboard/login`.
   - `NUXT_PUBLIC_AUTH_RESOURCE`: the same value as `NUXT_AUTH_AUDIENCE`.
...
10. Open `/dashboard/login`, sign in through FlareAuth, and verify the dashboard can call `/api/verify` with a Bearer access token.
